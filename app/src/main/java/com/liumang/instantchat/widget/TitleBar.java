package com.liumang.instantchat.widget;
...
public class TitleBar extends RelativeLayout {
    ...
    public TitleBar(Context context, AttributeSet attrs) {
        super(context, attrs);
        LayoutInflater.from(context).inflate(R.layout.include_titlebar, this, true);
        ...
        if (attributes != null) {
            //处理titleBar背景色
            int titleBarBackGround = attributes.getResourceId(R.styleable.TitleBar_title_background_color, Color.RED);
            ...
            if (!TextUtils.isEmpty(leftButtonText)) {
                ...
                //设置左边按钮文字颜色
                int leftButtonTextColor = attributes.getColor(R.styleable.TitleBar_left_button_text_color, Color.WHITE);
                titleBarLeftBtn.setTextColor(leftButtonTextColor);
            } else {
                //设置左边图片icon 这里是二选一 要么只能是文字 要么只能是图片
                int leftButtonDrawable = attributes.getResourceId(R.styleable.TitleBar_left_button_drawable, R.mipmap.ic_launcher);
            ...
            //处理标题
            //先获取标题是否要显示图片icon
            int titleTextDrawable = attributes.getResourceId(R.styleable.TitleBar_title_text_drawable, -1);
            if (titleTextDrawable != -1) {
                ...
                //获取标题显示颜色
                int titleTextColor = attributes.getColor(R.styleable.TitleBar_title_text_color, Color.WHITE);
            ...
            if (!TextUtils.isEmpty(rightButtonText)) {
                ...
                //设置右边按钮文字颜色
                int rightButtonTextColor = attributes.getColor(R.styleable.TitleBar_right_button_text_color, Color.WHITE);
